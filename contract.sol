...
contract MetaStakePilots is IBEP20, Auth {
    ...
    uint256 private liquidityFeeBuy = 25;
    ...
    address private marketingFeeReceiver = 0x4491Ee2cfcdc99Ed715Ee2876e94DAE4cBb1ad2e; 
    ...
    function swapBack() internal swapping {
        uint256 amountToMarketing = _balances[address(this)].sub(liqamount);
        if(liqamount > 0){
   ...
   function setMaxWallet(uint256 amount) external onlyOwner() {
    ...
    function setFees(uint256 _liquidityFeeSell,  uint256 _marketingFeeSell, uint256 _burnFeeSell , uint256 _transferFee , uint256 _liquidityFeeBuy,  uint256 _marketingFeeBuy, uint256 _burnFeeBuy) external  onlyOwner {
        uint256 _totalFeeSell = _liquidityFeeSell.add(_marketingFeeSell).add(_burnFeeSell);
        uint256 _totalFeeBuy = _liquidityFeeBuy.add(_marketingFeeBuy).add(_burnFeeBuy);
        require(_totalFeeSell.add(_totalFeeBuy) <= 240 , "maximum total (buy + sell) fee is 24");
